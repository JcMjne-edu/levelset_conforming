...
def msk_edge_angle_updated(nids,coord,threshold_angle):
  ...
  if (nids==np.roll(nids,1,axis=2)).sum(): #nids has duplicate
    print('Invalid nids',vs.shape,(nids==np.roll(nids,1,axis=2)).sum())
...
def get_new_face(nids,coord,threshold_angle=1.0):
  ...
  """
  if (nids==np.roll(nids,1,axis=2)).sum(): #nids has duplicate
    print('Invalid nids!',nids.shape,(nids==np.roll(nids,1,axis=2)).sum())
  ...
  m1=m1.any(axis=1) #(n2,3)
  m2=m2.any(axis=1) #(n2,3)
  uncommon1=np.argmin(m1,axis=1) #(n2,)
  uncommon2=np.argmin(m2,axis=1) #(n2,)
  ar=np.arange(i2.shape[0])
  nids_new=nids.copy() #(n2,2,3)
  nid_uncommon1=nids[ar,0,uncommon1] #(n2,)
  nid_uncommon2=nids[ar,1,uncommon2] #(n2,)
  nids_new[ar,1,i2[:,0]]=nid_uncommon1
  nids_new[ar,0,i1[:,1]]=nid_uncommon2
  
  edge_new=np.sort(np.array([nid_uncommon1,nid_uncommon2]).T,axis=1) #(n2,2)
...
def improve_aspect(connect,coord,threshold_angle=1.0):
  ...
  while True:
    ...
    nid_new,msk_update,_=get_new_face(nids,coord,threshold_angle)
    print(msk_update.sum(),"!")
...
def improve_aspect_fast(connect,coord,threshold_angle=1.0,n=3):
  """
  connect : (m,3)
  coord : (n,3)
  """
  ...
  msk_e_active=np.ones(u_edge.shape[0],dtype=bool)
  while True:
  #for i in range(n):
    ...
    msk_e_active=np.zeros(u_edge.shape[0],dtype=bool)
    msk_e_active[eid_updatable]=True
    ...
    msk_nid_update=np.zeros(nnode,dtype=bool)
    msk_nid_update[connect[fid_update].flatten()]=True
    ...
    arg_eid_face_updated=np.argmax(msk_diff_edge,axis=3).reshape(-1,6) #(neu,2*3)
    ...
    #update map_e2f
    map_e2f=np.arange(connect.shape[0]).repeat(3)[np.argsort(map_f2e.flatten())].reshape(-1,2) #(n,2)

    #update ar
    ar[fid_update.flatten()]=get_aspect_ratio(connect[fid_update.flatten()],coord)
    msk_e_active[eid_affected]=True
