...
py::tuple meshbuilder_aug(const Eigen::MatrixXd& coords, const Eigen::MatrixXi& connectivity,
    ...
    for (double x = bbox.xmin()-x_grid_size/2; x <= bbox.xmax()+x_grid_size; x += x_grid_size) {
        for (double y = bbox.ymin(); y <= bbox.ymax()+y_grid_size; y += y_grid_size) {
            for (double z = bbox.zmin()-z_grid_size/2; z <= bbox.zmax()+z_grid_size; z += z_grid_size) {
                Point p(x, y, z);
    ...
    for (double x = bbox.xmin()-x_grid_size/2; x < bbox.xmax(); x += x_grid_size) {
        for (double y = bbox.ymin(); y < bbox.ymax(); y += y_grid_size) {
            for (double z = bbox.zmin()-z_grid_size/2; z < bbox.zmax(); z += z_grid_size) {
                Point p000(x, y, z);
...
PYBIND11_MODULE(meshbuilder_aug, m) {
    m.def("meshbuilder_aug", &meshbuilder_aug, 
