...
def connect2adjoint(connect):
  ...
  """
  n=connect.max()+1
  ind=np.array([[0,1],[0,3],[0,4],[1,2],[1,5],[2,3],[2,6],[3,7],[4,5],[4,7],[5,6],[6,7]]) #(12,2)
  ind1,ind2=ind.T #(2,12)
  ind1=connect[:,ind1].flatten() #(12n,)
  ind2=connect[:,ind2].flatten() #(12n,)
  ind3=np.arange(n) #(n,)
  indices=np.array([np.concatenate((ind1,ind2,ind3)),np.concatenate((ind2,ind1,ind3))]).T #(24n,2)
  adjoint=sp.sparse.csr_matrix((np.ones(indices.shape[0],bool),(indices[:,0],indices[:,1])),shape=(n,n))
  return adjoint

def connect2adjoint_jax(connect):
  """
  Calculate adjoint matrix from node connectivity matrix in BCOO format\n
  connect : (n,8) array of integers
  """
