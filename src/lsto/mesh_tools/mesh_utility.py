...
def points_in_holes(coord,connect):
  ...
  verts=_return_vert(n_components,label_face,connect,stop_gradient(coord))
...
def _return_vert(n_components,label_face,connect,coord):
  ...
  ix=label_face[iz]
  out=jnp.zeros((n_components,n,3,3))
...
def make_adj(connect):
  """
  Create adjacency matrix from connectivity matrix
  connect : (n,3)
  """
  nnode=connect.max()+1
  indices=connect[:,[0,1,1,2,2,0]].reshape(-1,2)
