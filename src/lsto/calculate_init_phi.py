...
def ison_edge_jx(foot,v_ref):
  """
  Check if the point is on the edge of a triangle.
...
def penalty_dist(phi,weightrbf,connect_ls_str,coord_ls_str,norm,tri,nid_const,min_thickness=0.5,max_phi=-1e-5):
  _phi=weightrbf@phi
  ...
  penalty=term1+term2*1e2#+term3#+term4
  ...
  print(f"{stop_gradient(term1):.6f} {stop_gradient(term2):.6f} {stop_gradient(penalty):6f} {stop_gradient(min_dist.min()):.6f} {stop_gradient(min_dist.max()):.6f}")
  return penalty

def get_min_dist(phi,weightrbf,connect_ls_str,norm,tri,connect_ls_ex,vertices_ex):
  _phi=weightrbf@phi
  _phi_ex=redivision(_phi,connect_ls_str)
  numerator,denominator,offset=mat_phi2face_tetra(_nparray(_phi_ex),connect_ls_ex)
  mesh3d=((numerator@_phi_ex)@vertices_ex[offset])/(denominator@_phi_ex)[:,:,None]
  coord_index,_=mesh3d_to_coord_and_connect(stop_gradient(mesh3d),round_f=0)
  v=mesh3d.reshape(-1,3)[coord_index]
