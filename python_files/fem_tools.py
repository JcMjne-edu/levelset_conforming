...
import scipy as sp
from python_files.guyan_reduction import guyan_reduction
from python_files.custom_cholmod import custom_cholmod_mul
...
def MPL_penalty(K:BCOO,nid_constraint):
  ...
  indices=K.indices
  data=K.data
  #msk_remaining=jnp.where(~jnp.isin(indices,nid_flatten).any(axis=1))[0]
...
def _guyan_decomp_(M:BCOO,active_dim):
  """
  M : BCOO matrix (n,n)\n
  ...
  """
  data_cpu=M.data#jax.device_put(M.data,cpu)
  M_indices=M.indices # (nnz,2)
  n=M.shape[0]
  inactive_dim=jnp.setdiff1d(jnp.arange(n),active_dim,)
  data_ind=jnp.arange(data_cpu.shape[0]) # (nnz,)
  M_map_coo=sp.sparse.coo_matrix((data_ind,(M_indices[:,0],M_indices[:,1])),shape=M.shape)
  ...
  A_bcoo=BCOO((data_cpu[A_coo.data],A_ind),shape=A_coo.shape)
  B_coo=B_csc.tocoo(); B_ind=jnp.array([B_coo.row,B_coo.col]).T
  B_bcoo=BCOO((data_cpu[B_coo.data],B_ind),shape=B_coo.shape)
  C_coo=C_csc.tocoo(); C_ind=jnp.array([C_coo.row,C_coo.col]).T
  C_bcoo=BCOO((data_cpu[C_coo.data],C_ind),shape=C_coo.shape)
